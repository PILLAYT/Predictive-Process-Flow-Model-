...
PALLET_SIZE     = 42
POLL            = 0.01
# --- Cooling throttle (Press pause/resume) ---
COOLING_THROTTLE_HIGH = 1456   # pause presses when cooling WIP >= this
COOLING_THROTTLE_LOW  = 1239   # resume when cooling WIP <= this
    ...
    while True:
        if queues[wipo]:
            active_dest_pairs = [
                ...
                if h.machine_status.get(re.sub(r'^(WIPi_|WIPo_)', '', d), True)
            ]
...
#     queues.update(h.init_queues(cfg.network))
    ...
    # ─── Cooling-based Press throttle (hysteresis) ─────────────────────────
    press_allowed = True  # shared flag for all presses in Stage 1

    def cooling_throttle(env, high=COOLING_THROTTLE_HIGH, low=COOLING_THROTTLE_LOW):
        nonlocal press_allowed  # we mutate the closure var above
        while True:
            cooling_wip = len(storage_after_press)
            if press_allowed and cooling_wip >= high:
                press_allowed = False
                h.status_history.setdefault("PRESS_THROTTLE", []).append((env.now, False))
            elif (not press_allowed) and cooling_wip <= low:
                press_allowed = True
                h.status_history.setdefault("PRESS_THROTTLE", []).append((env.now, True))
            yield env.timeout(1.0)  # check once per sim minute

    env.process(cooling_throttle(env))
