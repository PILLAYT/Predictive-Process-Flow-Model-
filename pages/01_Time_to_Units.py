...
# st.set_page_config(
#     page_title="⏱️ Time → Units",
#     layout="wide",
#     initial_sidebar_state="collapsed",
# )

# # --- App banner (matches app.py) ---
...
#     <h1 class="page-title">⏱️ Time → Units</h1>
#     """,
#     unsafe_allow_html=True,
# )
...
# # Render HELP_TEXT but remove its first <h1>…</h1> (the “BB M0121 Plant Simulator” title)
# # ── Inject styling and help text (strip the big H1 inside HELP_TEXT) ─────────
# def strip_first_heading(md: str) -> str:
#     import re
#     s = md

#     # 1) remove first HTML <h1>…</h1> if present
#     s2 = re.sub(r"<h1[^>]*>.*?</h1>\s*", "", s, count=1, flags=re.I | re.S)
#     if s2 != s:
#         return s2

#     # 2) else remove first markdown H1 line that starts with '# '
#     s_lines = s.splitlines()
#     for i, line in enumerate(s_lines):
#         if line.lstrip().startswith("# "):      # markdown H1
#             del s_lines[i]
#             # also drop a blank line that usually follows
#             if i < len(s_lines) and s_lines[i].strip() == "":
#                 del s_lines[i]
#             return "\n".join(s_lines)
#     return s  # nothing to strip

# CLEAN_HELP = strip_first_heading(HELP_TEXT)
# st.markdown(CLEAN_HELP, unsafe_allow_html=True)
...
# ── Results (match Units → Time style) ─────────────────────────
results = st.session_state.get("sim_results")

if results:
    # KPI to match Units → Time style
    st.metric("Units in Dispatch (produced in this run)", results["fg6_count"])

    # Machine utilisation
    with st.expander("Machine utilisation"):
        dfu = results["df_util"].copy()
    ...
    with st.expander("Transport log"):
        st.dataframe(relabel_transport(results["df_move"]), use_container_width=True)

    # Final WIP count
    with st.expander("Final WIP count"):
        dff = results["df_final_wip"].copy()
        if "Node" in dff.columns:
            dff["Node"] = dff["Node"].map(NODE_LABELS).fillna(dff["Node"])
        st.dataframe(dff, use_container_width=True)

    # Unit-level summary (optional last panel to keep parity)
    with st.expander("Unit-level summary"):
        st.dataframe(results["df_units"], use_container_width=True)
